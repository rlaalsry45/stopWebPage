...
<%@ page language="java" import="egovframework.plani.template.cmm.utils.CheckPlusSafe" %>
<%@ page language="java" import="egovframework.plani.template.cmm.utils.EgovSessionCookieUtil" %>
<%@ page language="java" import="egovframework.plani.template.cmm.utils.EgovSessionCookieUtil" %>
<%@ page language="java" import="egovframework.plani.template.cmm.vo.UserAuthVO" %>
<%@ page language="java" import="egovframework.plani.custom.memsvc.vo.IdentificationVO" %>

<%@ page language="java" import="egovframework.plani.custom.memsvc.web.MemberController" %>
...
<%
	...
	*********************************************************************************************************************************************/
	...
	String sSiteCode				= "G603";			// IPIN 서비스 사이트 코드		(NICE평가정보에서 발급한 사이트코드)
	String sSitePw					= "ECO2dna1";			// IPIN 서비스 사이트 패스워드	(NICE평가정보에서 발급한 사이트패스워드)
	...
	*/
	...
	userAuth.setVirtualNo(sVNumber);
	userAuth.setAuthType("IPIN");
	userAuth.setBirthDate(sBirthDate);
	userAuth.setAge(safe.getAge(sBirthDate));
	
	IdentificationVO idfVO = new IdentificationVO();
	idfVO.setUser_nm(sName);
	idfVO.setConfirm_way("1");
	idfVO.setIdkey(sDupInfo);
...
<head>
	...
	<meta http-equiv=Refresh content=0;url="/memuser/authPageView.do?pg_ret_type=<%= sReservedParam2 %>">
</head>
