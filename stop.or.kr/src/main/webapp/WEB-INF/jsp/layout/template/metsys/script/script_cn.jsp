...
$(document).ready(function(){
	...
	{ 
		...
		//게시물 삭제시 확인
		$("a.deleteCardNews").click(function()
		{
		...
		$('form.ajax-form-submit_cn').AjaxFormSubmit("","저장 하시겠습니까?","",save_success);
...
function file_attach_check()
{
	...
	{
		$('form.ajax-form-submit_cn').submit();
	}
}
...
{
	var form = document.reqForm;
	fn_egov_move_action(form, '/metsys/cardnews/cardNewsManList.do?srch_menu_nix=${param.srch_menu_nix}' + fn_egov_get_argstr());
}
...
function save_success(result)
{
	...
	{	
   		if(result.data.cn_seq)
			{
	   			cmmfn_set_uploadifykeys('file_upload', 'CARD_NEWS', result.data.cn_seq, 1, 'GNR');
