...
<c:if test="${menu.value.use_yn eq 'Y' }">
	...
	<c:if test="${!empty menu.value._children_}">
		...
		<c:if test="${sub_menu.value.use_yn eq 'Y' }">
			...
			<li <c:if test="${sub_menu.value.menu_nix eq param.srch_menu_nix}">class="bgcolor1"</c:if>><a href="${a_link}" <c:if test="${!empty target}">target="${target}"</c:if>>${sub_menu.value.menu_nm}</a></li>
