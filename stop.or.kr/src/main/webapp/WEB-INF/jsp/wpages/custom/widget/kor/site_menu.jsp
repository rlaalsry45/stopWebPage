...
<c:if test="${menu.value.use_yn eq 'Y' }">
	...
	<c:if test="${menu.value.use_yn eq 'Y'}">
		<li>
			<c:choose>
				<c:when test="${menu.value.menu_nm == path.get(0).menu_nm}">
					<a style="text-decoration: underline;" href="${a_link}" <c:if test="${!empty target}">target="${target}"</c:if>>${menu.value.menu_nm}</a>
				</c:when>
				<c:otherwise>
					<a href="${a_link}" <c:if test="${!empty target}">target="${target}"</c:if>>${menu.value.menu_nm}</a>
				</c:otherwise>
			</c:choose>
			...
			<div class="depth2">
				...
				<c:forEach var="sub_menu" items="${menu.value._children_}">
						<c:if test="${sub_menu.value.use_yn eq 'Y' }">
							...
							<li <c:if test="${sub_menu.value.menu_nix eq param.srch_menu_nix}">class="bgcolor1"</c:if>><a href="${a_link}" <c:if test="${!empty target}">target="${target}"</c:if>>${sub_menu.value.menu_nm}<c:if test="${!empty target}">&nbsp;&nbsp;<img src="../../images/kor/main/icon-open.jpg" alt="새창열림"/></c:if></a>
								...
								<ul class="depth3 bul1">
									<c:forEach var="sub_menu_3depth" items="${sub_menu.value._children_}">
										<c:if test="${sub_menu_3depth.value.use_yn eq 'Y' }">
											...
											<li <c:if test="${sub_menu_3depth.value.menu_nix eq param.srch_menu_nix}">class="bgcolor1"</c:if>><a href="${a_link}" <c:if test="${!empty target}">target="${target}"</c:if>>${sub_menu_3depth.value.menu_nm}<c:if test="${!empty target}">&nbsp;&nbsp;<img src="../../images/kor/main/icon-open.jpg" alt="새창열림"/></c:if></a></li>
											</c:if>
										</c:forEach>
									</ul>
								</c:if>
