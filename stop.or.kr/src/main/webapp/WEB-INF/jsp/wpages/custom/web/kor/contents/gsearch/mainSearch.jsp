...
<div class="search-result">
	...
	<c:forEach var="board" items="${boardListCont}" varStatus="boardStatus">
		<h2 class="title1">${board.brd_nm}<span class="txt1">(총 <strong class="point3">${board.searchCount}</strong>건)</span></h2>
		<ul class="bul1">
			<c:forEach var="result" items="${resultListCont}" varStatus="status">
	...
	<c:forEach var="board" items="${boardListCms}" varStatus="boardStatus">
		<h2 class="title1">${board.brd_nm}<span class="txt1">(총 <strong class="point3">${board.searchCount}</strong>건)</span></h2>
		<ul class="bul1">
					<c:set var="index" value="1"/>
					<c:forEach var="result" items="${resultListCms}" varStatus="status">
						<c:if test="${board.brd_id eq result.brd_id and index le 10}">
