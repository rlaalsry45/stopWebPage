		...
		<c:if test="${tabmenu.use_yn eq 'Y' }">
			...
			<c:choose>
				<c:when test="${fn:indexOf(tabmenu.menu_link,'srch_menu_nix=') < 0}">
					<c:set var="a_link" value="${pageContext.request.contextPath}${tabmenu.menu_link}${tag_url}srch_menu_nix=${tabmenu.menu_nix}&srch_mu_lang=CDIDX00023"/>
				</c:when>
				<c:otherwise>
					<c:choose> 
						...
						<c:otherwise>
							<c:set var="a_link" value="${pageContext.request.contextPath}${tabmenu.menu_link}${tag_url}&srch_mu_lang=CDIDX00023"/>
						</c:otherwise>
					</c:choose>
				</c:otherwise>
			</c:choose>
