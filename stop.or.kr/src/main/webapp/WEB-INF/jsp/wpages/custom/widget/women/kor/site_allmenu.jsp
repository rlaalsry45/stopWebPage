...
<c:if test="${!empty menu}">
	<li><a href="${pageContext.request.contextPath}/women">여성폭력Zoomin</a>
	<div class="depth2">
		<ul>
				...
				<c:if test="${menu.value.use_yn eq 'Y'}">
					...
					<c:choose>
						<c:when test="${fn:indexOf(menu.value.menu_link,'srch_menu_nix=') < 0}">
							<c:set var="a_link" value="${pageContext.request.contextPath}${menu.value.menu_link}${tag_url}srch_menu_nix=${menu.value.menu_nix}&srch_mu_site=CDIDX00005"/>
						...
						<c:otherwise>
							<c:choose> 
								...
								<c:otherwise>
									<c:set var="a_link" value="${pageContext.request.contextPath}${menu.value.menu_link}&srch_mu_site=CDIDX00005"/>
								</c:otherwise>
							</c:choose>
						</c:otherwise>
					</c:choose>			
								...
								<c:choose>
									...
									<c:otherwise>
										<c:choose> 
											...
											<c:otherwise>
												<c:set var="a_link" value="${pageContext.request.contextPath}${sub_menu.value.menu_link}&srch_mu_site=CDIDX00005"/>
											</c:otherwise>
										</c:choose>
									</c:otherwise>
								</c:choose>
