...
<c:forEach var="sidemenu" items="${sidemenu}">
	...
	<c:choose>
		<c:when test="${fn:indexOf(sidemenu.value.menu_link,'srch_menu_nix=') < 0}">
			<c:set var="a_link" value="${pageContext.request.contextPath}${sidemenu.value.menu_link}${tag_url}srch_menu_nix=${sidemenu.value.menu_nix}&srch_mu_lang=CDIDX00023"/>
		</c:when>
		<c:otherwise>
			<c:choose> 
					...
					<c:choose>
						...
						<c:otherwise>
							<c:choose> 
								...
								<c:otherwise>
									<c:set var="a_link" value="${pageContext.request.contextPath}${sub_menu_3depth.value.menu_link}${tag_url}&srch_mu_lang=CDIDX00023"/>
								</c:otherwise>
							</c:choose>
						</c:otherwise>
					</c:choose>
