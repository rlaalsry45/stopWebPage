...
<c:forEach var="sidemenu" items="${sidemenu}">
	...
	<c:choose>
		<c:when test="${fn:indexOf(sidemenu.value.menu_link,'srch_menu_nix=') < 0}">
			<c:set var="a_link" value="${pageContext.request.contextPath}${sidemenu.value.menu_link}${tag_url}srch_menu_nix=${sidemenu.value.menu_nix}"/>
		</c:when>
		<c:otherwise>
			<c:choose> 
	...
	<c:if test="${!empty sidemenu.value._children_}">
		...
		<c:if test="${sub_menu.value.use_yn eq 'Y'}">
			...
			<!--  외부링크일경우 치환 -->
			<c:if test="${fn:indexOf(sub_menu.value.menu_link,'www') > 0}">
				<c:set var = "a_link" value = "${fn:replace(a_link, pageContext.request.contextPath, '')}" />
			</c:if>
					
			<c:set var="liClass" value=""/>		
			<c:if test="${sub_menu.value.use_yn eq 'Y'}">
				<c:if test="${!empty sub_menu.value._children_}">
					<c:set var="liClass" value="type1"/>
				</c:if>
				<li class="${liClass}${activeClass}"><a href="${a_link}" target="${target}" <c:if test="${target eq '_blank'}">title="새창열림"</c:if>>${sub_menu.value.menu_nm}</a>
			</c:if>
			...
			<c:if test="${!empty sub_menu.value._children_}">
				...
				<c:if test="${sub_menu_3depth.value.use_yn eq 'Y'}">
					...
					<!--  외부링크일경우 치환 -->
					<c:if test="${fn:indexOf(sub_menu_3depth.value.menu_link,'www') > 0}">
						<c:set var = "a_link" value = "${fn:replace(a_link, pageContext.request.contextPath, '')}" />
					</c:if>
					...
					<c:choose>
						...
						<li class="${thirdActiveClass}"><a href="${a_link}" target="${target}" <c:if test="${target eq '_blank'}">title="새창열림"</c:if>>${sub_menu_3depth.value.menu_nm}</a></li>
					</c:if>
				</c:forEach>
				</ul>
			</c:if>
