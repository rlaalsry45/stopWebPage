						...
						<c:otherwise>
								...
								<td class="left">
									...
									<%-- <c:choose>
										<c:when test="${SESS_USR_LV le 2}">
											<form:input path = "wdt" title="작성일" value="${boardthmVO.wdt}"/>										
										</c:when>
										<c:otherwise>
											<c:out value="${boardthmVO.wdt}"/>
										</c:otherwise>
								...
								<tr>
									...
									<td class="left">
										<form:select path="ctgry_idx" class="select">
											<form:option value="">분류를 선택해주십시오 </form:option>
								...
								<tr>
									...
									<td class="left">
										<c:if test="${SESS_BRD_INFO.use_notice eq 'Y'}">
											<input type="checkbox" id="is_notice" name="is_notice" value="N" <c:if test="${boardthmVO.is_notice eq 'Y'}">checked="checked"</c:if>/><label for="is_notice">공지 설정</label>
										</c:if>
										<c:if test="${SESS_BRD_INFO.use_scrt eq 'Y'}">
											<input type="checkbox" id="is_scrt" name="is_scrt" value="N" <c:if test="${boardthmVO.is_scrt eq 'Y'}">checked="checked"</c:if>/><label for="is_scrt">비공개 설정</label>
										</c:if>
									</td>
								</tr>
							</c:if>	
							...
							<c:if test="${extra1_array.extra1_use eq 'Y'}">
								<tr>
									...
									<td>
										<input type="text" name="extra1" value="<c:out value='${boardarticleVO.extra1}'/>"/>
									</td>
								</tr>
							</c:if>
							<c:if test="${extra2_array.extra2_use eq 'Y'}">
								<tr>
									...
									<td>
										<input type="text" name="extra2"  value="<c:out value='${boardarticleVO.extra2}'/>"/>
									</td>
								</tr>
							</c:if>
							<c:if test="${SESS_BRD_INFO.off_viewmode eq '2'}">
								<tr>
									<th scope="row"><c:if test="${boardthmVO.edomweivgp eq 'A'}"><span class="point01">*</span></c:if> <label for="add_info_04">비밀번호</label></th>
									<td class="left">												
							...
							<c:if test = "${SESS_BRD_INFO.use_kogl eq 'Y'}">
								<tr>
									...
									<td class="form">
										<strong>※ 공공저작물 자유이용을 적용하면 이 정보가 공공누리 사이트로 전송됩니다. ${boardthmVO.kogl}</strong></br>
										<c:choose>
				...
				<div class="btn_area_right">
					...
					<c:choose>
						...
						<c:otherwise>
							...
							<input type="submit" class="btn_line" value="<spring:message code="BOARD.SAVE" />">
						</c:otherwise>		
					</c:choose>
