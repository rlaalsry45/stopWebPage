...
<form:form name="reqForm" commandName="boardarticleVO">
		...
		<ul class="tabmenu_blue"> 
			<c:choose>
				<c:when test="${empty param.srch_ctgry_idx}">
					<li class="on"><a href="${pageContext.request.contextPath}/brdartcl/boardarticleList.do?brd_id=${SESS_BRD_INFO.brd_id}&&srch_menu_nix=${param.srch_menu_nix}">전체</a></li>
				</c:when>
				<c:otherwise>
			...
			<c:forEach var="result" items="${ctgryList}" varStatus="status">
				...
				<c:choose>
					<c:when test="${result.ctgry_idx eq 'CTIDX00022' or result.ctgry_idx eq 'CTIDX00023'}">
						<c:choose>
							<c:when test="${result.ctgry_idx eq param.srch_ctgry_idx}">
								<li class="on"><a href="${pageContext.request.contextPath}/brdartcl/boardarticleList.do?brd_id=${SESS_BRD_INFO.brd_id}&srch_ctgry_idx=${result.ctgry_idx}&srch_menu_nix=${param.srch_menu_nix}"><c:out value="${result.ctgry_nm}"/> 공지사항</a></li>
							</c:when>
							<c:otherwise>
					...
					<c:otherwise>
						<c:choose>
							<c:when test="${result.ctgry_idx eq param.srch_ctgry_idx}">
								<li class="on"><a href="${pageContext.request.contextPath}/brdartcl/boardarticleList.do?brd_id=${SESS_BRD_INFO.brd_id}&srch_ctgry_idx=${result.ctgry_idx}&srch_menu_nix=${param.srch_menu_nix}"><c:out value="${result.ctgry_nm}"/></a></li>
							</c:when>
							<c:otherwise>
	...
	<div id="detail_content">
		...
		<div class="articles_search">
			...
			<!-- 
			<div class="inquiry">
				<span class="icon_inquiry">문의 : 채정숙 (T. 042-612-8512)</span>
			</div>
			-->
		</div>
		<!-- //페이지 작성 -->
					...
					<c:forEach var="notice" items="${noticeList}" varStatus="status">
						<tr class="row_point">
								...
								<td class="ellipsis"><nobr>
										...
										<a href="${pageContext.request.contextPath}/brdanm/boardanonimView.do?srch_menu_nix=${param.srch_menu_nix}&brd_id=${SESS_BRD_INFO.brd_id}&cont_idx=${notice.cont_idx}&edomweivgp=R">
								...
								<td class="ellipsis">
									<nobr>
										...
										<c:choose>
											...
											<c:otherwise>
												<a href="${pageContext.request.contextPath}/brdanm/boardanonimView.do?srch_menu_nix=${param.srch_menu_nix}&brd_id=${SESS_BRD_INFO.brd_id}&cont_idx=${result.cont_idx}&edomweivgp=R">
													<c:if test="${result.grp_lv > 1}">
														<img src="${pageContext.request.contextPath}/images/template/cmm/icon_reply.gif" alt="Reply Symbol" />
													</c:if> 
													<span id="spanListSubject${status.index}" title="${result.subject}"><c:out value="${result.subject}" /></span> 
								...
								<td>
									...
									<c:choose>
										<c:when test="${SESS_BRD_INFO.off_viewmode eq '2'}">
											${result.add_info_01}
											<c:if test="${empty result.add_info_01}">${result.writer_nm}</c:if>
										</c:when>
										<c:otherwise>
