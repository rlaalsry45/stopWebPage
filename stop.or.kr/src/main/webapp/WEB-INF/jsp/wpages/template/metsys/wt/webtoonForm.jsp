...
<form:form name="reqForm" commandName="webtoonVO" action="${cutil:getUrl('/metsys/webtoon/webtoonSave.do', '', true)}" class="ajax-form-submit_wt" method="post" enctype="multipart/form-data">
...
<div id="divMainArticle">
		...
		<table class="tstyle_view">
			...
			<tr style="display:none">
				...
				<td>
					<input type="radio" name="is_notice" id="is_notice_y" value="Y" <c:if test="${ fn:trim(webtoonVO.is_notice) eq 'Y'}">checked="checked"</c:if>><label for="is_notice_y">사용</label>
					<input type="radio" name="is_notice" id="is_notice_n" value="N" <c:if test="${ fn:trim(webtoonVO.is_notice) eq 'N'}">checked="checked"</c:if>><label for="is_notice_n">미사용</label>
				</td>
			</tr>
			<tr>
				...
				<td>
					<select name="category">
						<c:forEach var="cresult" items="${webtooncategorylist}">
							<option value="${cresult.cn_seq}">${cresult.cn_title}</option>
						</c:forEach>
					</select>
				</td>
			</tr>
