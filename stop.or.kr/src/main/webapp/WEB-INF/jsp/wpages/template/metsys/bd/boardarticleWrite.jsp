...
<form:form name="reqForm" commandName="boardarticleVO" enctype="multipart/form-data" class="board-ajax-form-submit">
								...
								<tr>
									...
									<td class="left">
										<c:if test="${SESS_BRD_INFO.use_notice eq 'Y'}">
											<input type="checkbox" id="is_notice" name="is_notice" value="N" <c:if test="${boardarticleVO.is_notice eq 'Y'}">checked="checked"</c:if>/><label for="is_notice">공지 설정</label>
										</c:if>
										<c:if test="${SESS_BRD_INFO.use_scrt eq 'Y'}">
											<input type="checkbox" id="is_scrt" name="is_scrt" value="N" <c:if test="${boardarticleVO.is_scrt eq 'Y'}">checked="checked"</c:if>/><label for="is_scrt">비공개 설정</label>
										</c:if>
									</td>
								</tr>
							</c:if>								
						...
						<c:otherwise>
								...
								<td class="left">
									...
									<%-- <c:choose>
										<c:when test="${SESS_USR_LV le 2}">
											<form:input path = "wdt" title="작성일" value="${boardarticleVO.wdt}"/>										
										</c:when>
										<c:otherwise>
											<c:out value="${boardarticleVO.wdt}"/>
										</c:otherwise>
							...
							<c:if test="${SESS_BRD_INFO.use_notice eq 'Y' or SESS_BRD_INFO.use_scrt eq 'Y'}">
								<tr>
									...
									<td class="left">
										<c:if test="${SESS_BRD_INFO.use_notice eq 'Y'}">
											<input type="checkbox" id="is_notice" name="is_notice" value="Y" <c:if test="${boardarticleVO.is_notice eq 'Y'}">checked="checked"</c:if>/><label for="is_notice">공지 설정</label>
										</c:if>
										<c:if test="${SESS_BRD_INFO.use_scrt eq 'Y'}">
											<input type="checkbox" id="is_scrt" name="is_scrt" value="N" <c:if test="${boardarticleVO.is_scrt eq 'Y'}">checked="checked"</c:if>/><label for="is_scrt">비공개 설정</label>
										</c:if>
							...
							<c:if test="${SESS_BRD_INFO.off_viewmode eq '2'}">
								<tr>
									<th scope="row"><c:if test="${boardarticleVO.edomweivgp eq 'A'}"><span class="point01">*</span></c:if> <label for="add_info_04">비밀번호</label></th>
									<td class="left">												
							...
							<c:if test = "${SESS_BRD_INFO.use_kogl eq 'Y'}">
								<tr>
									...
									<td class="form">
										<strong>※ 공공저작물 자유이용을 적용하면 이 정보가 공공누리 사이트로 전송됩니다. ${boardarticleVO.kogl}</strong></br>
										<c:choose>
					...
					<c:choose>
						<c:when test="${boardarticleVO.edomweivgp eq 'R'}">
							...
							<c:if test="${RWAUTH eq 'RWD'}">
								...
								<a href="${pageContext.request.contextPath}/brdartcl/boardarticleMoveView.do?brd_id=${SESS_BRD_INFO.brd_id}&srch_menu_nix=${param.srch_menu_nix}&cont_idx=${boardarticleVO.cont_idx}&edomweivgp=M">수정</a>
