			...
			<table class="tstyle_view" summary="${SESS_BRD_INFO.brd_nm} 대한 상세보기입니다.">
				...
				<tr>
					...
					<td colspan="3" class="left">
						<strong><span class="txt_bold">${cutil:htmlspecialchars(boardarticleVO.subject)}</span></strong>
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="title">작성자</label></th>
					<td class="title">
						<strong><span class="txt_bold">${cutil:htmlspecialchars(boardarticleVO.writer_nm)}</span></strong>
					</td>
					<th scope="row"><label for="title">작성일</label></th>
					<td class="title">
						<strong><span class="txt_bold">${cutil:htmlspecialchars(boardarticleVO.wdt)}</span></strong>
					</td>
				</tr>
				<tr>
				...
				<ul class="nextPrev_list">
					<li><strong class="prev_list">이전글</strong>
						<c:choose> 
							...
							<c:otherwise>
								<a href="javascript:fn_egov_brd_view('${boardarticleVO.pre_atcl}')">${cutil:htmlspecialchars(boardarticleVO.pre_subj)}</a>
							</c:otherwise>
						</c:choose>
					</li>
					<li><strong class="next_list">다음글</strong> 
						<c:choose> 
							...
							<c:otherwise>
								<a href="javascript:fn_egov_brd_view('${boardarticleVO.next_atcl}')">${cutil:htmlspecialchars(boardarticleVO.next_subj)}</a>
							</c:otherwise>
						</c:choose>
					</li>
				</ul>
					...
					<c:when test="${SESS_USR_LV le 1000}">
						<dl class="comment_write">
							<dt>
								...
								<span><input type="hidden" name="writer_nm" id="writer_nm" value="${cutil:htmlspecialchars(SESS_USR_INFO.user_nm)}"/> <input type="hidden" name="brd_pwd" id="brd_pwd" type="password" size="15" /></span>
							</dt>
							<dd>
						...
						<ul class="comment_write" id="comment_write">
							<c:forEach var="result" items="${replyList}" varStatus="status">
								<li id="dlidx_${result.reply_idx}">
									<strong class="comment_write_name"><c:out value="${result.writer_nm}"/></strong>
									<span class="comment_write_date"><c:out value="${result.wdt}"/></span>
									<div class="comment_write_content" id="spanReplyArea_${result.reply_idx}"><c:out value="${result.reply_cont}"/></div>
		...
		<div class="btn_area_right">
			...
			<c:if test="${(SESS_BRD_INFO.reply_gb eq 'A' or SESS_BRD_INFO.reply_gb eq 'B') and (RWAUTH eq 'RW' or RWAUTH eq 'RWD')}">
				...
				<c:if test="${boardarticleVO.is_notice eq 'N' and SESS_USR_LV le 100}">
					<a href="${pageContext.request.contextPath}/brdartcl/boardarticleMoveView.do?brd_id=${SESS_BRD_INFO.brd_id}&srch_menu_nix=${param.srch_menu_nix}&cont_idx=${boardarticleVO.cont_idx}&edomweivgp=P" class="btn_line"><spring:message code="BOARD.REPLY" /></a>
			...
			<%-- <c:if test="${RWAUTH eq 'RW'}"> --%>
				...
				<a href="${pageContext.request.contextPath}/brdartcl/boardarticleMoveView.do?brd_id=${SESS_BRD_INFO.brd_id}&srch_menu_nix=${param.srch_menu_nix}&cont_idx=${boardarticleVO.cont_idx}&edomweivgp=M" class="btn_line">수정</a>
			<%-- </c:if>
