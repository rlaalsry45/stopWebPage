				...
				<table class="tstyle_view" summary="${SESS_BRD_INFO.brd_nm}에 대한 글쓰기입니다.">
					...
					<c:choose>
						...
						<c:otherwise>
							...
							<c:if test="${SESS_BRD_INFO.brd_gb_sub eq 'B'}">
								<tr>
									<!-- 액티비 표시 -->
									<th scope="row"><label for="title">분류</label></th>
									<td class="left">
										<form:select path="active" class="select">
							...
							<tr>
								...
								<td colspan="2" class="form" id="editorWrapper">
									<c:choose>
										...
										<c:otherwise>
											<c:choose>
												<c:when test="${SESS_BRD_INFO.brd_id eq 'BDIDX_lkc50Gv082E33tD0Zsv2n0' or  SESS_BRD_INFO.brd_id eq 'BDIDX_gQ7i747CKk4zN0cU1qxlqk'}">
													<div id="editor_frame"></div>
													<form:textarea path="brd_cont" class="display_none"></form:textarea>
												</c:when>
												<c:otherwise>
						...
						<c:when test="${boardarticleVO.edomweivgp eq 'R'}">
							<c:if test="${(SESS_BRD_INFO.reply_gb eq 'A' or SESS_BRD_INFO.reply_gb eq 'B') and (RWAUTH eq 'RW' or RWAUTH eq 'RWD')}">
								...
								<c:if test="${boardarticleVO.is_notice eq 'N'}">
									<a href="${pageContext.request.contextPath}/brdanm/boardanonimMoveView.do?srch_menu_nix=${param.srch_menu_nix}&brd_id=${SESS_BRD_INFO.brd_id}&cont_idx=${boardarticleVO.cont_idx}&edomweivgp=P" class="btn_line">답글</a>
								</c:if>
							</c:if>
