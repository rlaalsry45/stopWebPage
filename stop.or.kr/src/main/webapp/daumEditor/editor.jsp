...
<%
	...
	String editor_type = (request.getParameter("editor_type") == null) ? "all" : request.getParameter("editor_type");
	String return_form_name = (request.getParameter("return_form_name") == null) ? "frm" : request.getParameter("return_form_name"); 
	String return_textarea_name = (request.getParameter("return_textarea_name") == null) ? "content" : request.getParameter("return_textarea_name");
			...
			<div id="tx_sidebar" class="tx-sidebar">
				<div class="tx-sidebar-boundary">
					...
					<ul class="tx-bar tx-bar-left tx-nav-attach">
						...
						-->
						<% if("all".equals(editor_type) || "image".equals(editor_type) ){ %>
						<li class="tx-list">
						...
						<!-- 이미지 첨부 버튼 끝 -->
						<% } %>
						<% if("all".equals(editor_type) || "file".equals(editor_type) ){ %>
						<li class="tx-list">
...
<script type="text/javascript">
	...
	var return_form_name = '<%=return_form_name%>'; 
	var return_textarea_name = '<%=return_textarea_name%>';
	var config = {
		...
		form: return_form_name+"", /* 등록하기 위한 Form 이름 */
 ...
 */
	...
	function setForm(editor) {
        ...
        textarea.name = return_textarea_name;
        ...
        for (i = 0; i < images.length; i++) {
            ...
            if (images[i].existStage) {
                ...
                str_atchFileId += images[i].data.atchFileId;
            }
        }
        input = document.createElement('input');
        input.type = 'hidden';
        input.name = 'editorAtchFileId';
        input.id = 'editorAtchFileId';
        //input.value = images[i].data.imageurl;  // 예에서는 이미지경로만 받아서 사용
        input.value = str_atchFileId; 
        form.createField(input);
        }
		
        /* 현 전자정부프레임워크에선 이미지만 사용
