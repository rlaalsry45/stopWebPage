...
public class URLEncoder {
  ...
  static {
     ...
     *
     * RFC 2396 states: ----- Data characters that are allowed in a URI but do not have a reserved
     * purpose are called unreserved. These include upper and lower case letters, decimal digits,
   ...
   * encoding scheme. This method uses the supplied encoding scheme to obtain the bytes for unsafe
   * characters.
   * <p>
    ...
    for (int i = 0; i < s.length(); i++) {
      ...
      if (dontNeedEncoding.get(c)) {
        ...
        try {
          ...
          writer.write(c);
          /*
           * If this character represents the start of a Unicode surrogate pair, then pass in two
           * characters. It's not clear what should be done if a bytes reserved in the surrogate
           * pairs range occurs outside of a legal surrogate pair. For now, just treat it as if it
