...
public class EgovComCrossSiteHndlr extends BodyTagSupport {
  ...
  /*
   * (One almost wishes XML and JSP could support "anonymous tags," given the amount of trouble we
   * had naming this one!) :-) - sb
   */
  ...
  /**
   * Constructs a new handler. As with TagSupport, subclasses should not provide other constructors
   * and are expected to call the superclass constructor.
   */
  ...
  /**
   * Outputs <tt>text</tt> to <tt>pageContext</tt>'s current JspWriter. If <tt>escapeXml</tt> is
   * true, performs the following substring replacements (to facilitate output to XML/HTML pages):
   *
   * & -> &amp; < -> &lt; > -> &gt; " -> &#034; ' -> &#039;
   *
  ...
  public static void out2(PageContext pageContext, boolean escapeXml, Object obj)
      throws IOException {
    JspWriter w = pageContext.getOut();
  ...
  /**
   *
   * Optimized to create no extra objects and write directly to the JspWriter using blocks of
   * escaped and unescaped characters
   *
   */
  ...
  /**
   * 2011.10.10 cmd 라인상에서 편의제공을 위해 제공, 필요없을시 삭제하여도 무방함 public static void main(String[] args) throws
   * IOException {
   * 
   ...
   * 
   * try{ System.out.println("TRY TEST 1"); throw new Exception(); }catch(Exception e){
   * System.out.println("TRY TEST 2"); }finally{ System.out.println("TRY TEST 3");
   * 
   * } }
   */
}
