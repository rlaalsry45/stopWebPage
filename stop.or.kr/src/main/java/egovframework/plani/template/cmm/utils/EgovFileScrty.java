/**
 * Class Name : EgovFileScrty.java Description : Base64인코딩/디코딩 방식을 이용한 데이터를 암호화/복호화하는 Business
...
public class EgovFileScrty {
  ...
  /**
   * 전화번호 데이터를 복호화하는 기능 (맨 마지막 자리만)
   *
   * @param String data 복호화할 데이터
   * @return String result 복호화된 데이터
   * @exception Exception
   */
  public static String decodeTelLast(String data) throws Exception {
    String tel = new String(decodeBinary(data));
   ...
   * @exception Exception
   */
  public static String decodeTelAll(String data) throws Exception {
    String retv = "";
    String tel = new String(decodeBinary(data));
    tel = tel.trim();
    ...
    if (tel.startsWith("02")) {
      if (tel.length() == 9) {
        retv = tel.substring(0, 2);
        retv += "-";
        retv += tel.substring(2, 5);
        retv += "-";
        retv += tel.substring(5, tel.length());
      } else if (tel.length() == 10) {
        retv = tel.substring(0, 2);
        retv += "-";
        retv += tel.substring(2, 6);
        retv += "-";
        retv += tel.substring(6, tel.length());
      } else {
        retv = tel;
      }
      ...
      if (tel.length() == 10) {
        retv = tel.substring(0, 3);
        retv += "-";
        retv += tel.substring(3, 6);
        retv += "-";
        retv += tel.substring(6, tel.length());
      } else if (tel.length() == 11) {
        retv = tel.substring(0, 3);
        retv += "-";
        retv += tel.substring(3, 7);
        retv += "-";
        retv += tel.substring(7, tel.length());
      } else {
        retv = tel;
      }
    }
  ...
  public static void main(String[] args) {
    try {
      System.out.println(EgovFileScrty.encryptPassword("0000"));
      System.out.println(EgovFileScrty.encryptMD5("khg49930"));
    } catch (Exception e) {
