...
public class MetsysController {
	...
	public Object login(@Valid LoginVO loginVO, BindingResult result, HttpServletRequest request,
		...
		if (chkVO == null) {
			...
			return responseResultHelper.error("", MessageHelper.getMessage("NO-MATCHING-MEMBER-INFORMATION"), null, null);
		} else {
			...
			if (sess_or == "1") {
				...
				return responseResultHelper.error(null, "다른 컴퓨터에서 로그인 중 입니다.\n다시 로그인 해주시길 바랍니다.", "/metsys/metsysLogout.do?srch_menu_nix=DF13IQ59");
			}
			...
			if (!chkVO.getUser_id().equals("nimdasys")) {
				...
				if (chkVO.getPass_mdt_d() < -90) {
					...
					// 90일넘어가면 로그인 미처리
					return responseResultHelper.error(null, "장기간 비밀번호를 변경하지않아 접속이 중단되었습니다. 관리자에 문의하여 주십시오.", "/metsys/metsysLogout.do?srch_menu_nix=DF13IQ59");
					
				} else if (chkVO.getPass_mdt_d() < -80) {
					
					int remain_day = (-80 - chkVO.getPass_mdt_d());
					return responseResultHelper.error(null, remain_day + "일 내 비밀번호를 변경하지 않으면 계정 접속이 중단됩니다.", "/metsys/metsysLogout.do?srch_menu_nix=DF13IQ59");
	...
	public Map<String, Object> metsysOtpValidation(HttpServletRequest request, Model model) throws Exception {
    	...
    	try{
			...
			if (!otpResult) {
				...
				manlogService.insertManlog(request, vo.getUser_id(), "로그인관리", "로그인 : " + vo.getUser_id(), "M");
				memberService.updateMemberLastlogin(vo);
